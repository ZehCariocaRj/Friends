let PRUDPPacketError = require('./errors/packet.error.class');

let RC4 = require('./rc4.class');
let cipher = new RC4('CD&ML');
...
class PRUDPPacket {
    ...
    unpack() {
        ...
        let buffer = Buffer.from(this.raw, 'hex');

        switch (this.version) {
            case 0:
                let type_flags = buffer.subarray(0x2, 0x4).readUInt16LE(),
                    type = type_flags & 0xF,
                    flags = (type_flags & 0xFFF0) >> 4;
    ...
    construct_packet(json) {
		...
		let packet_header = new Buffer();
		let packet_payload = new Buffer();
		let packet_checksum = new Buffer();
		switch (this.version) {
            case 0:
                packet_header = Buffer.concat([Buffer.from(json.ports.source + json.ports.destination, 'hex'),
					/*put packet type and flags here, as well as other stuff*/);
                break;
            case 1:
                // Not implemented
                // Will use `unpack_header` method
    ...
    calc_checksum(checksum, packet) {
        ...
        for (let i=0;i<FloorDiv(packet.length, 4);i++) {
            tuple.push(packet.readUInt32LE(pos, true));
            pos += 4;
        }

        let sum = tuple.reduce((a, b) => {
                return a + b;
            }, 0),
            temp = (sum & 0xFFFFFFFF) >>> 0;
