...
class PRUDPPacket {
	...
	unpack() {
		...
		switch (this.version) {
			case 0:
				...
				if (data.type == TYPES.DATA && !data.flags.includes('ACKN')) {
					if (!data.size) {
				...
				if(data.type == TYPES.DATA && data.payload.encrypted != null){
					...
					if(data.payload.decrypted != null){
						data.payload.rmc.type = Buffer.from(0);
						data.payload.rmc.size = data.payload.decrypted.subarray(0x00,0x03);
						data.payload.rmc.protocol_id = data.payload.decrypted[0x04] | 0x80;
						data.payload.rmc.call_id = data.payload.decrypted.subarray(0x05,0x08);
						data.payload.rmc.method_id = data.payload.decrypted.subarray(0x09,0x0B);
					}
				}
	...
	construct_packet(json) {
		...
		switch (json.version) {
			case 0:
				packet_header = Buffer.from(json.ports.source + json.ports.destination); //adds vports
				let flag = 0;
				if(json.flags.includes('ACKN'){
					flag += 1;
				}
				if(json.flags.includes('RELIABLE'){
					flag += 2;
				}
				if(json.flags.includes('NEED_ACK'){
					flag += 4;
				}
				if(json.flags.includes('HAS_SIZE'){
					flag += 8;
				}
				if(json.flags.includes('MULTI_ACK'){
					flag += 200;
				}
				const flag_buf = Buffer.from(flag);
				packet_header += Buffer.concat([Buffer.from(flag_buf[0x02] + json.type + flag_buff.subarray(0x00,0x01)])); //adds flags & type
				packet_header += Buffer.concat([Buffer.from(json.session_id), Buffer.from(json.signature)]); //adds session_id & sig
				//needs sequence, packet-specific data, payload, and checksum
				break;
			case 1:
				// Not implemented
				// Will use `unpack_header` method
				break;
			default:
				throw new PRUDPPacketError(`Invalid or unimplemented packet version ${json.version} provided`);
		}
	}
